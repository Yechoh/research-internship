...
setProject path name cur
	...
	>>- \settings ->			(case mprojtxt of
		Nothing ->					(callProcess "Create project" [] settings.dirCpm ["project",(cur </> "Temp" </> (toproj name)),"create"] Nothing 
									>>|- return ())
		(Just projtxt) ->			(writeToFile (cur </> "Temp" </> (toproj name)) projtxt 
									>>|- return ())
								)
/*
...
cpmtask iclloc = 
			...
			accWorld ('SP'.callProcess dirCmd ["/c",dirCpm,toproj iclloc,"1>templog.txt"] Nothing)
