...
//	# fontNames		= lfilter fixed fontNames
	...
	# watt		=		[ WindowPen		[PenBack LightGrey]
...
where
	...
	setBoxCol cb1id cb2id cb3id cb4id (ls=:{cur},ps)
		# (cId,col) = case cur of
						TXT	-> (cb1id,ls.TWC_LS.txt)
						CMT	-> (cb2id,ls.TWC_LS.cmt)
						MOD	-> (cb3id,ls.TWC_LS.mod)
						BCK	-> (cb4id,ls.TWC_LS.bck)
	...
	updateTW win (ls,ps)
		...
		# (font,ps)		= safeOpen {fName = ls.TWC_LS.fn, fSize = ls.TWC_LS.fs, fStyles = []} ps
		# twi			= TW_SetSync sync twi
		# twi			= TW_SetFnt font twi
		# ps			= appPLoc (setTypeWinInfo twi) ps
		# (_,ps)		= maybe_type_win_message2 (appFontInfo (fi_update ls)) ps
