...
where
	...
	controlToHandles (FlexText text ftId atts) ps
		...
		# imp = {addLS = inils,addDef=CustomControl siz (look inils) [ControlId cid:satts] :+: Receiver2 ftId (recfun cid) []}
	...
	where
		...
		recfun cId (FlexTextSet ft) ((ls,gs),ps)
			# ls = {ls & ft = ft}
			# ps = appPIO (setControlLooks [(cId,True,(True,look ls))]) ps
		...
		recfun _ FlexPenGet ((ls=:{pn},gs),ps)
			= (FlexPenOK pn,((ls,gs),ps))
		look {ft=text,pn=pen} sel {newFrame} pic
