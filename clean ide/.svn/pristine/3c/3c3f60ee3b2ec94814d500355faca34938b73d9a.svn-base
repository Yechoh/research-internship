...
getDirEntry also_get_file_name platform files
	#	((fileName, fileSizeTuple, lastModifiedTuple, isDirectory, isReadOnly), files)
		...
		fileSize	= { low32=fileSizeLow, high32=fileSizeHigh }
...
instance == DirError
  where
	(==) NoDirError x			= case x of 
									NoDirError		-> True
									_				-> False
	(==) DoesntExist x			= case x of 
									DoesntExist		-> True
									_				-> False
	(==) BadName x				= case x of
									BadName			-> True
									_				-> False
	(==) NotEnoughSpace x		= case x of
									NotEnoughSpace	-> True
									_				-> False
	(==) AlreadyExists x		= case x of
									AlreadyExists	-> True
									_				-> False
	(==) NoPermission x			= case x of
									NoPermission	-> True
									_				-> False
	(==) MoveIntoOffspring x	= case x of
									MoveIntoOffspring-> True
									_				-> False
	(==) MoveAcrossDisks x		= case x of
									MoveAcrossDisks	-> True
									_				-> False
	(==) NotYetRemovable x		= case x of
									NotYetRemovable	-> True
									_				-> False
	(==) OtherDirError x		= case x of
									OtherDirError	-> True
									_				-> False
...
instance toInt		Integer64	// will truncate values>2^31-1
  where
	toInt {low32, high32}
		|	high32>0
			= 2^31-1
