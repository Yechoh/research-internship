...
import IdeState
from typewin import updateTypeWindow, tw_safe_close, Typer, TypeWinInfo
from typeatt import typeWinKeyboard, typeWinMouse
from errwin import updateErrorWindow, ew_safe_close
from messwin import showInfo, Info, Level1, Level2, Level3
from projwin import pm_update_project_window
from PmAbcMagic import PatchSystemABC, ParseABCDependencies`, ModuleDate, PatchABCDates, Combined
import PmFileInfo
import PmDirCache
...
step intr (DLink ds=:{ok, newpaths, fileinfo, libsinfo, modpaths, abccache, project, continue}) ps
	...
	# (abcPathsCache,ps)		= case genabc of
									True	-> let (sysdate,ps`) = fmodified` (full_sys0 +++ {dirseparator} +++ (MakeObjPathname tp System)) ps in
												(DC_Update ((MakeObjPathname tp System),full_sys0,sysdate) abcPathsCache,ps`)
												// need to check if line above actually works now...
									False	-> (abcPathsCache,ps)
	| not ok
		= continue False newpaths False fileinfo libsinfo modpaths project intr (abccache, ps)
	...
	# stdl						= standardStaticLibraries lo.method
	# stdo						= standardObjectFiles ao.stack_traces ao.profiling
...
*/
	...
	# ofiles					= Concat clmodpaths ofiles
...
//	# (genabc,ps) 				= CheckLibsOutOfDate genabc lfiles ps
	# (ood,ps)					= case lo.method of
									LM_Static  -> CheckExecOutOfDate genabc False execpath (makeCompPathFromExecPath execpath) fileinfo project ps
									LM_Eager   -> CheckExecOutOfDate genabc True  execpath (makeCompPathFromExecPath execpath) fileinfo project ps
									LM_Dynamic -> (True,ps)
	| not ood
...
//*/
	# static_info = 
		...
		// Martijn wil dat we hier een evt. StaticClientChannel weer uitfilteren...
...
//			# (ok,ps) = DynLink (app_path +++ {dirseparator} +++dynlstr) prj_path app_path ps
//			# ps = okNotice ["Dynamic linked: "+++toString ok] ps

		// wil geen exe gaan executen!!! De aanroep van DynLink hoort eigenlijk in de exe fase thuis...
		// probleem is nog dat ide de wijzigingen van DynLink in het project zo negeert...
		= continue ok newpaths ok fileinfo libsinfo modpaths project intr (abccache, ps)

	# (linkstr,ps)				= getCurrentLink ps
	# (startupdir,ps)			= getStup ps
		
// wil eigenlijk hier pas controleren of exe ood...
// ao ok
// lfiles ok
// ofiles ok
// sfiles ok
// dus exe date cachen met gebruikte ao en lfiles,ofiles,sfiles dates...
// kan nu al verbeteren door in Younger naast clean objecten ook overige mee te nemen
...
where
	fmodified` fname ps
		# ({DATE | yy,mm,dd,h,m,s},ps)
								= accFiles (FModified fname) ps
		= (({year`=yy,month`=mm,day`=dd,dayNr`=0},{hours`=h,minutes`=m,seconds`=s}),ps)
...
//XXX	# ps						= showInfo line ps
	...
	# write_module_times		= version == 918
...
ProcessCompilerMsg compileOrCheckSyntax co path abcpath CompilerOK fileinfo dircache abccache project ps0
	...
	#! ((ok,dircache),ps)	= case version of
								918 -> ((True,dircache),ps)
								_	-> accFiles (PatchABCDates abcpath dircache) ps
	| not ok
...
CheckABCOutOfDate sys path abccache fileinfo project ps
	...
	# mn						= GetModuleName path
	# cg_opt					= if sys
									(not (PR_SysUptoDate project))		// gaat nu niet goed met switch tussen wel/geen time profiling...???
...
makeCompPathFromExecPath execpath
	= RemoveSuffix` execpath +++ ".dat"

CheckExecOutOfDate :: !Bool !Bool !Pathname !Pathname !FileInfoCache !Project !*(PSt General) -> *(Bool,*PSt General)
CheckExecOutOfDate gen checkcomp execpath comppath fileinfo project ps
	| gen
		= (True,ps)
	#	({be_verbose},ps)	= getPrefs ps
	#	execname			= RemovePath execpath
	| not (PR_ExecUpToDate project)	// dit gaat helemaal niet goed!!
		#	lines			= if be_verbose
	...
	// moet hier controleren of dates == stored dates of zo?
	// hmmm, gaat mis met twee kopieen van zelfde project.
	// Mischien maar gewoon zo laten...
	...
	| not checkcomp
		= (False,ps)
	// check complement out of date...
	# compname			= RemovePath comppath
	# (date, ps)		= accFiles (FModified comppath) ps
	# link				= not date.exists || Older_Date date youngest
	| link
		#	lines		= if be_verbose
							(if date.exists
								(Level3 ["[" +++ compname +++ ",]: is older than .obj files. Linking new complement."])
								(Level3 ["[" +++ compname +++ ",]: does not exist. Linking new complement."])
								)
							(Level3 [])
		= (True, verboseInfo be_verbose lines ps)
	= (False,ps)
...
// rewrite to accu version or cont version?
