...
where
	...
	where
		...
		//	Remove elements:
		//	Remove all:
		receiver (FInCloseAllItems) ((listboxState=:{pen,items,selection,lineHeight,initHeight},ls),ps)
			# listboxState	= {listboxState & items=[], aitems = [], selection=[]}
			# (newDomain,ps)= calcControlDomain pen [] ps
			# newLook		= customlook listboxState
			# ps			= appPIO (seq
								[ setControlViewDomain listboxState.listboxId.fcontrolId newDomain
								, setControlLooks [(listboxState.listboxId.fcontrolId,True,(True,newLook))]
								]) ps
			= (FOutCloseAllItems,((listboxState,ls),ps))
		
		// Set control pen:
		receiver (FInSetPen newpen) ((listboxState=:{items,pen},ls),ps)
			...
			# ps			= appPIO (seq
								[ setControlViewDomain listboxState.listboxId.fcontrolId newDomain
								, setControlLooks [(listboxState.listboxId.fcontrolId,True,(True,newLook))]
								, setControlScrollFunction listboxState.listboxId.fcontrolId Vertical (altScrollFunction Vertical lineHeight)
								]) ps
...
customlook {items,selection,pen,lineHeight,initHeight} _ {newFrame} pict
