...
//import Platform
import System.FilePath, System.OS
...
quoted_string s = IF_WINDOWS (win_quoted_string s) (mac_quoted_string s)
...
RemovePath path
	| found	= path % (inc position, last);
			= path;
	where 
		(found,position)= FindLastChar pathSeparator path last;
		last			= dec (size path);
...
RemoveFilename path 
	# (found,position)	= FindLastChar pathSeparator path (dec (size path));
	| found
		= path % (0, dec position);
		= path;
...
replace_prefix_path old_prefix_path new_prefix_path path
	...
	| equal_path_prefix old_prefix_path path
		= new_prefix_path +++ (toString pathSeparator) +++ path % (inc (size old_prefix_path), dec (size path));
		= path;

IsFullPathname :: !Pathname -> Bool;
IsFullPathname name = LastChar pathSeparator name (dec (size name)) >= 0;

MakeFullPathname :: !Pathname !String -> Pathname;
MakeFullPathname path name =  path +++ (toString pathSeparator) +++ name;

replace_dots_by_dir_separators :: !{#Char} -> *{#Char};
replace_dots_by_dir_separators module_name
	= {if (c=='.') pathSeparator c \\ c<-:module_name}
...
equal_path_prefix prefix string
	| lengths<=lengthpre
		= False;
		= string.[inc lengthpre]== pathSeparator && string % (0,lengthpre)==prefix;
	where 
		lengths		= dec (size string);
